...
echo "fincat line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "fincat/agent")
core_bus=$(count_top_level_py_lines "fincat/bus")
core_config=$(count_top_level_py_lines "fincat/config")
core_cron=$(count_top_level_py_lines "fincat/cron")
core_heartbeat=$(count_top_level_py_lines "fincat/heartbeat")
core_session=$(count_top_level_py_lines "fincat/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "fincat/agent/tools")
extra_skills=$(count_skill_lines "fincat/skills")
extra_api=$(count_recursive_py_lines "fincat/api")
extra_cli=$(count_recursive_py_lines "fincat/cli")
extra_channels=$(count_recursive_py_lines "fincat/channels")
extra_utils=$(count_recursive_py_lines "fincat/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under fincat/agent"
echo "  - tools/ is counted separately from fincat/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, fincat.py, root files"

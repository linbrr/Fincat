"""WebSocket server channel: fincat acts as a WebSocket server and serves connected clients."""
...
from fincat.config.schema import Base
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      fincat and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-Fincat-Auth: <secret>``.
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-Fincat-Auth") or headers.get("x-fincat-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._on_raw_message_handlers: list[Any] = []
    ...
    async def _connection_loop(self, connection: Any) -> None:
        ...
        try:
            ...
            # Notify connect handlers (e.g. TopicDispatcher for initial push)
            logger.info("websocket: firing {} on_connect handler(s) for chat_id={}", len(self._on_connect_handlers), chat_id)
            for handler in self._on_connect_handlers:
                try:
                    result = handler(chat_id)
                    if asyncio.iscoroutine(result):
                        await result
                    logger.info("websocket: on_connect handler done for chat_id={}", chat_id)
                except Exception as e:
                    logger.warning("websocket on_connect handler error: {}", e)
            ...
            async for raw in connection:
                ...
                # Let raw message handlers intercept (e.g. action: request_topics)
                handled = False
                if raw.strip().startswith("{") and self._on_raw_message_handlers:
                    try:
                        data = json.loads(raw)
                        if isinstance(data, dict):
                            for handler in self._on_raw_message_handlers:
                                try:
                                    result = handler(data, chat_id)
                                    if asyncio.iscoroutine(result):
                                        result = await result
                                    if result:
                                        handled = True
                                    ...
                                    logger.warning("websocket raw_message handler error: {}", e)
                    except json.JSONDecodeError:
                        pass
                if handled:
                    continue
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if connection is None:
            ...
            return
        # Chart data is pre-serialized JSON, send directly
        if msg.metadata.get("_chart_data"):
            await self._safe_send(msg.chat_id, msg.content, label=" chart ")
            return
    ...
    async def send_delta(
        ...
        if self._connections.get(chat_id) is None:
            logger.debug("websocket.send_delta: no connection for chat_id={}", chat_id)
            return
        meta = metadata or {}
    ...
    async def broadcast_topics(self, topics: list[Any]) -> None:
        """Broadcast topics to all connected WebSocket clients."""
        if not self._connections or not topics:
            return
        payload = {
            "event": "topics",
            "topics": [
        ...
        raw = json.dumps(payload, ensure_ascii=False)
        dead: list[str] = []
        for chat_id, conn in self._connections.items():
            try:
                await conn.send(raw)
            except ConnectionClosed:
                dead.append(chat_id)
            except Exception as e:
                logger.error("websocket broadcast_topics failed: {}", e)
        for cid in dead:
            self._connections.pop(cid, None)

    async def send_topics(self, topics: list[Any]) -> None:
        """ChannelSender protocol implementation for TopicDispatcher."""
        await self.broadcast_topics(topics)

    async def push_topics(self, topics: list[Any]) -> None:
        """Push topics to all connected clients (called by TopicCache)."""
        await self.broadcast_topics(topics)

    def on_connect(self, handler: Any) -> None:
        """Register a callback invoked when a new client connects.

        handler(chat_id: str) -> None
        """
        self._on_connect_handlers.append(handler)

    def on_raw_message(self, handler: Any) -> None:
        """Register a callback for raw inbound JSON messages.

        handler(data: dict, chat_id: str) -> bool
        Return True if the message was handled (skip normal processing).
        """
        self._on_raw_message_handlers.append(handler)

    async def send_topics_to_connection(self, chat_id: str, topics: list[Any]) -> None:
        """Send topics to a specific connection (for initial push on connect)."""
        conn = self._connections.get(chat_id)
        if conn is None:
            logger.warning("send_topics_to_connection: no conn for chat_id={}", chat_id)
            return
        if not topics:
            logger.info("send_topics_to_connection: no topics to send")
            return
        logger.info("send_topics_to_connection: sending {} topic(s) to chat_id={}", len(topics), chat_id)
        payload = {
            "event": "topics",
            "topics": [
                {
        ...
        try:
            ...
            logger.error("websocket send_topics_to_connection failed: {}", e)

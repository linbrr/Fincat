...
If you discover a security vulnerability in fincat, please report it by:
...
chmod 600 ~/.fincat/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.fincat/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.fincat/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run fincat with a dedicated user account
...
- Keep authentication data in `~/.fincat/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for fincat and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash fincat
   sudo -u fincat fincat gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.fincat
   chmod 600 ~/.fincat/config.json
   chmod 700 ~/.fincat/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.fincat/logs/fincat.log
   ```
...
- **Chat history is stored locally** - protect the `~/.fincat` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.fincat/logs/fincat.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/fincat/security/advisories
- Release Notes: https://github.com/HKUDS/fincat/releases

"""Tests for the fincat programmatic facade."""
...
from fincat.client import Fincat, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Fincat.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Fincat.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("fincat.config.loader.load_config") as mock_load, \
         patch("fincat.client._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = Fincat.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from fincat.config.schema import Config
    from fincat.client import _make_provider
...
def test_import_from_top_level():
    from fincat import Fincat as N, RunResult as R
    assert N is Fincat

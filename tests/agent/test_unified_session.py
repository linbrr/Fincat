...
from fincat.command.builtin import cmd_new, register_builtin_commands
from fincat.command.router import CommandContext, CommandRouter
from fincat.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("fincat.agent.loop.SessionManager"), \
         patch("fincat.agent.loop.SubagentManager") as MockSubMgr, \
         patch("fincat.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates fincat onboard)."""
        from fincat.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from fincat.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from fincat.agent.loop import UNIFIED_SESSION_KEY
        from fincat.command.builtin import cmd_stop

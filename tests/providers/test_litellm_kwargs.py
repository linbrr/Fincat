...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("fincat.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
    ...
    assert headers["X-OpenRouter-Title"] == "fincat"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    assert headers["X-OpenRouter-Title"] == "Fincat Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("FINCAT_STREAM_IDLE_TIMEOUT_S", "0")

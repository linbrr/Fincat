...
async def test_brave_search(monkeypatch):
    ...
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="fincat", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("fincat.agent.tools.web.DDGS", MockDDGS, raising=False)
    import fincat.agent.tools.web as web_mod

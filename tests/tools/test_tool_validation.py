...
from fincat.agent.tools import (
    ArraySchema,
    IntegerSchema,
    ObjectSchema,
    Schema,
...
def test_exec_extract_absolute_paths_captures_home_paths() -> None:
    ...
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "~/.fincat/config.json" in paths
    assert "~/out.txt" in paths


def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.fincat/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.fincat/config.json" in paths
...
def test_exec_guard_blocks_quoted_home_path_outside_workspace(tmp_path) -> None:
    tool = ExecTool(restrict_to_workspace=True)
    error = tool._guard_command('cat "~/.fincat/config.json"', str(tmp_path))
    assert error == "Error: Command blocked by safety guard (path outside working dir)"
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("fincat.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import fincat.agent.tools.shell as shell_mod

...
from fincat.cli import commands
from fincat.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("fincat.cli.commands._PROMPT_SESSION", mock_session), \
         patch("fincat.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("fincat.cli.commands.PromptSession") as MockSession, \
         patch("fincat.cli.commands.FileHistory") as MockHistory, \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("fincat.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:

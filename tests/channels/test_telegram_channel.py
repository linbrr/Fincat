...
from fincat.channels.telegram import TELEGRAM_REPLY_CONTEXT_MAX_LEN, TelegramChannel, _StreamBuf
from fincat.channels.telegram import TelegramConfig
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("fincat.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "fincat.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import fincat.channels.telegram as tg_mod
...
async def test_on_error_logs_network_issues_as_warning(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "fincat.channels.telegram.logger.warning",
        lambda message, error: recorded.append(("warning", message.format(error))),
    )
    monkeypatch.setattr(
        "fincat.channels.telegram.logger.error",
...
async def test_send_delta_stream_end_splits_oversized_reply() -> None:
    ...
    from fincat.channels.telegram import TELEGRAM_MAX_MESSAGE_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("fincat.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "fincat.channels.telegram.validate_url_target",
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "fincat.channels.telegram.get_media_dir",

...
try:
    from fincat.channels import dingtalk
...
import fincat.channels.dingtalk as dingtalk_module
from fincat.channels.dingtalk import DingTalkChannel, FincatDingTalkHandler
from fincat.channels.dingtalk import DingTalkConfig
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = FincatDingTalkHandler(channel)
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "fincat.config.paths.get_media_dir",

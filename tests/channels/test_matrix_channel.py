...
from fincat.channels.matrix import _build_matrix_text_content

import fincat.channels.matrix as matrix_module
from fincat.bus.events import OutboundMessage
from fincat.bus.queue import MessageBus
from fincat.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from fincat.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("fincat.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "fincat.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("fincat.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "fincat.channels.matrix.asyncio.create_task", _fake_create_task
    )

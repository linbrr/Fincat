...
Use fincat programmatically — load config, run the agent, get results.
...
### `Fincat.from_config(config_path?, *, workspace?)`

Create a `Fincat` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.fincat/config.json`. |
...
async def main():
    bot = Fincat.from_config(workspace="/my/project")

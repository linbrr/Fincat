...
Build a custom fincat channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of fincat (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

fincat discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `fincat gateway` starts, it scans:

1. Built-in channels in `fincat/channels/`
2. External packages registered under the `fincat.channels` entry point group
...
```
fincat-channel-webhook/
├── fincat_channel_webhook/
...
```python
# fincat_channel_webhook/__init__.py
from fincat_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# fincat_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "fincat-channel-webhook"
version = "0.1.0"
dependencies = ["fincat", "aiohttp"]

[project.entry-points."fincat.channels"]
webhook = "fincat_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
fincat plugins list      # verify "Webhook" shows as "plugin"
fincat onboard           # auto-adds default config for detected plugins
```

Edit `~/.fincat/config.json`:
...
```bash
fincat channels login <channel_name>
fincat channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `fincat onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `fincat.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `fincat.config.schema.Base`:
...
Override `default_config()` so `fincat onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `fincat-channel-{name}` | `fincat-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `fincat_channel_{name}` | `fincat_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/fincat-channel-webhook
cd fincat-channel-webhook
pip install -e .
fincat plugins list    # should show "Webhook" as "plugin"
fincat gateway         # test end-to-end
```
